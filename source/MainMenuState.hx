...
{
	public static var VSDantesVersion:String = '1.0.0';
	...
	var storyModeThingy:FlxSprite;

	var freeplaySus:FlxSprite;

	var credsToYall:FlxSprite;

	var optionsHmm:FlxSprite;
	...
	override function create()
	{
		...
		var yScroll:Float = Math.max(0.25 - (0.05 * (optionShit.length - 4)), 0.1);
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('backgroundthingy'));
		...
		var menuItem:FlxSprite = new FlxSprite(30, 70);
		...
		//	storyModeThingy = new FlxSprite(700, 50);
		//	storyModeThingy.frames = Paths.getSparrowAtlas('mainmenu/menu_storymode');
		//	storyModeThingy.scale.set(0.6, 0.6);
		//	storyModeThingy.animation.addByPrefix('idle', "storymode basic", 24);
		//	storyModeThingy.animation.addByPrefix('selected', "storymode white", 24);
		//	storyModeThingy.animation.play('idle');
		//	storyModeThingy.ID = 0;
		//	storyModeThingy.angle = 7;
		//	menuItems.add(storyModeThingy);
		//	storyModeThingy.scrollFactor.set();
		//	storyModeThingy.antialiasing = ClientPrefs.globalAntialiasing;
		//	storyModeThingy.updateHitbox();
		//	storyModeThingy.screenCenter();
		//	storyModeThingy.x -= 385;
		//	storyModeThingy.y -= 200;

			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(50, 250);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
			...
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(-150, 400);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
			...
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(770, 160);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
			...
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(770, 360);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 64, 0, "VS Dantes v" + VSDantesVersion, 12);
